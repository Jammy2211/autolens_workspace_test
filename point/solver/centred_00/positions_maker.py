...
"""Use a `PointSolver` which does not use grid upscaling."""

solver = al.PointSolver(grid=grid, use_upscaling=False, upscale_factor=2)
...
for i in range(iters):
    ...
    """Save the `Tracer` and `Positions` so they can be used for testing other `PointSolver` settings."""
