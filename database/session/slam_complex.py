...
settings_autofit = af.SettingsSearch(
    path_prefix=path.join("database", "session", "slam_complex"),
...
analysis = al.AnalysisImaging(
    dataset=masked_imaging, hyper_dataset_result=source_lp_results.last
)
...
source_pix_results = slam.source_pix.run(
    ...
    pixelization=al.mesh.VoronoiBrightnessImage,
...
"""
analysis = al.AnalysisImaging(
    dataset=masked_imaging, hyper_dataset_result=source_pix_results.last
)
...
lens = agg.model.galaxies.lens
agg_query = agg.query(lens.mass == al.mp.PowerLaw)
samples_gen = agg_query.values("samples")
print(
    "Total Samples Objects via `PowerLaw` model query = ",
    len(list(samples_gen)),
    "\n",
)
