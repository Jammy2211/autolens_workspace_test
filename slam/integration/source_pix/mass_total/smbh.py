...
setup_mass = al.SetupMassTotal(mass_prior_model=al.mp.Isothermal)
setup_source = al.SetupSourceParametric(
    bulge_prior_model=al.lp.Sersic, disk_prior_model=None, envelope_prior_model=None
)

pipeline_source_lp = al.SLaMPipelineSourceParametric(
...
pipeline_source_pix = al.SLaMPipelineSourceInversion(setup_source=setup_source)
...
default of an `PowerLaw` in this example.
...
setup_mass = al.SetupMassTotal(mass_prior_model=al.mp.PowerLaw)
...
slam = al.SLaM(
    ...
    setup_hyper=hyper,
    pipeline_source_lp=pipeline_source_lp,
    pipeline_source_pix=pipeline_source_pix,
...
from slam.with_lens_light import source__parametric, mass__total, light__parametric
from slam.with_lens_light import source_pix
...
source_pix = source_pix.make_pipeline(
    slam=slam, settings=settings, source_lp_results=source_results
)
source_results = source_pix.run(dataset=imaging, mask=mask)
