...
def with_lens_light(
    ...
    if settings_autofit.unique_tag in ["slacs0841+3824"]:
        nlive = 600

    if settings_autofit.unique_tag in ["slacs1213+6708"]:

        model.galaxies.source.bulge.centre_0 = af.UniformPrior(
            lower_limit=-0.27, upper_limit=-0.23
        )
        model.galaxies.source.bulge.centre_1 = af.UniformPrior(
            lower_limit=-0.23, upper_limit=-0.21
        )
        model.galaxies.source.bulge.effective_radius = af.UniformPrior(
            lower_limit=0.12, upper_limit=0.13
        )
