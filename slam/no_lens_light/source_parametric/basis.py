...
settings_autofit = af.SettingsSearch(
    path_prefix=path.join("slam", "mass_total__source_parametric", "basis"),
    number_of_cores=1,
    session=None,
)
...
gaussian_list = af.Collection(af.Model(al.lp_linear.EllGaussian) for _ in range(10))

for i, gaussian in enumerate(gaussian_list):

    gaussian.centre = gaussian_list[0].centre
    gaussian.elliptical_comps = gaussian_list[0].elliptical_comps
...
"""
analysis = al.AnalysisImaging(dataset=imaging, hyper_dataset_result=source_results.last)
