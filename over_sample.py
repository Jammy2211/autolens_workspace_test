...
signal_to_noise_map = image_sub_1 / masked_dataset.noise_map

mat_plot_2d = aplt.MatPlot2D(
    output=aplt.Output(
        path=file_path, filename=f"signal_to_noise_map_sub_1", format="png"
    )
)
plotter = aplt.Array2DPlotter(array=signal_to_noise_map, mat_plot_2d=mat_plot_2d)
plotter.figure_2d()
...
grid = al.Grid2D.from_mask(mask=mask, over_sample_size=2)
...
# radial_grid = np.sqrt(grid[:,0] ** 2 + grid[:,1] ** 2)
#
# fractional_accuracy.native[radial_grid > 3.8] = 0.0

# fractional_accuracy[fractional_accuracy > 1.0] = 1.0 / fractional_accuracy[fractional_accuracy > 1.0]
