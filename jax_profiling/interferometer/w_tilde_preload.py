...
w_tilde = al.util.inversion_interferometer_numba.w_tilde_curvature_preload_interferometer_from(
...
print("NUMPY W TILDE", time.time() - start)
...
w_compact = w_compact_curvature_interferometer_from(
    noise_map_real=noise_map,
    uv_wavelengths=uv_wavelengths,
    grid_size=grid.mask.shape_native_masked_pixels[0],
...
print(w_compact.shape)

w_tilde_jax_compact = w_tilde_via_compact_from(
    w_compact=w_compact,
    native_index_for_slim_index=grid.mask.slim_to_native_tuple
)

print(np.abs(w_tilde_jax_compact - w_tilde).max())