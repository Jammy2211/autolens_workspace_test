...
def detection(
    ...
    search_no_subhalo = af.Nautilus(
        name="subhalo[1]_mass[total_refine]", **settings_autofit.search_dict, n_live=200, verbose=True,
    )
...
def sensitivity_mapping_imaging(
    ...
    """
    simulate_function = SimulateImaging(mask=mask_2d, psf=psf_2d)

    """
    We next specify the search used to perform each model fit by the sensitivity mapper.
    """
    search = af.Nautilus(
        name="subhalo__sensitivity", **settings_autofit.search_dict, n_live=50
    )
...
def sensitivity_mapping_interferometer(
    ...
    def simulate_function(instance):
        ...
        """
        grid = al.Grid2DIterate.uniform(
            shape_native=real_space_mask_2d.shape_native,
            pixel_scales=real_space_mask_2d.pixel_scales,
            fractional_accuracy=0.9999,
            sub_steps=[2, 4, 8, 16, 24],
        )
