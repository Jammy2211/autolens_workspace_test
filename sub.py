...
source_dict['bulge'] = al.lp.Sersic(
    centre=(0, 0),
    ell_comps=al.convert.ell_comps_from(axis_ratio=1, angle=0),             #Create a source galaxy with a centered sersic light profile
    intensity=0.3,
...
lens_NFW_dict = {}
lens_NFW_dict['subhalo'] = al.mp.NFWTruncatedMCRLudlowSph(centre=(0, 0), mass_at_200=10**12)
lens_NFW_galaxy = al.Galaxy(redshift=0.5, **lens_NFW_dict)

tracer_1 = al.Tracer.from_galaxies(galaxies=[lens_NFW_galaxy, source_galaxy])                   #Create a lens galaxy with a NFW profile of mass 10^12 centered at 0,0
...
tracer_2 = al.Tracer.from_galaxies(galaxies=[lens_SIS_galaxy, source_galaxy])               #Create a lens galaxy with a SIS profile of centered at 0,0
...
lens_combined_1_dict['subhalo'] = al.mp.NFWTruncatedMCRLudlowSph(centre=(-2.0, 0), mass_at_200=10**12)

lens_combined_1_galaxy = al.Galaxy(redshift=0.5, **lens_combined_1_dict)

tracer_3 = al.Tracer.from_galaxies(galaxies=[lens_combined_1_galaxy, source_galaxy])          #When combining the two profiles togethetr some small perturbation is visibile but it's not as pronounced as I would have thought for such a massive halo
...
lens_combined_2_dict['subhalo'] = al.mp.IsothermalSph(centre=(-2.0, 0), einstein_radius=0.2)

lens_combined_2_galaxy = al.Galaxy(redshift=0.5, **lens_combined_2_dict)

tracer_4 = al.Tracer.from_galaxies(galaxies=[lens_combined_2_galaxy, source_galaxy])          #If I instead use a smaller SIS profile instead of a NFW profile I get a much more pronoucned effect on the Einstein ring
