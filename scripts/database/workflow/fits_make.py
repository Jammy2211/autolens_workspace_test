...
for i in range(2):
    ...
    search = af.Nautilus(
        path_prefix=path.join("results_folder_fits"),
        name="results",
...
results we require.
"""
workflow_path = Path("output") / "results_folder_fits" / "workflow_make_example"
...
agg = Aggregator.from_directory(
    directory=path.join("output", "results_folder_fits"),
)
