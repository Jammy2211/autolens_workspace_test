...
The same initial fit creating results in a folder called `results_folder_csv` is therefore used.
...
"""
workflow_path = Path("output") / "results_folder_csv" / "workflow_make_example"
...
The `results_folder_csv` contained two model-fits to two different datasets, meaning that each `add_variable` 
...
This will output in your current working directory (e.g. the `autolens_workspace/output.results_folder_csv`) 
