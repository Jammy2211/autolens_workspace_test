...
Dataset: MGE lens bulge + IsothermalSph mass on the build interferometer dataset.
...
"""
image_mesh = al.image_mesh.Overlay(shape=(8, 8))
...
mesh_rect = al.mesh.RectangularAdaptImage(shape=(8, 8))
...
for source_name, model, has_inversion in source_runs:
    ...
    assert (
        sub_path / "tracer.png"
    ).exists(), f"{source_name}/tracer.png missing"
